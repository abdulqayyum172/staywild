...
export default function Buy() {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      try {
        const res = await fetch(`${API_URL}/buy/properties?limit=100`);
        if (!res.ok) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-20">
      <section
        className="relative w-full overflow-hidden py-20 text-center text-white shadow-xl"
        style={{
      ...
      >
        ...
        <div className="relative z-10 mx-auto max-w-3xl px-5">
          <p className="mb-3 text-sm font-semibold uppercase text-emerald-200">Purchase listings</p>
          <h1 className="text-4xl font-bold md:text-6xl">Buy with clear market context.</h1>
          <p className="mt-4 text-base leading-7 text-white/85 md:text-lg">
            Review verified homes, compare locations, and continue only when the property matches your budget and brief.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto mt-10 max-w-6xl rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        <div className="mb-4 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-xl font-semibold text-slate-950">Filter properties</h2>
            <p className="text-sm text-slate-500">
              {loading ? "Loading properties..." : `Showing ${filtered.length} purchase options`}
            </p>
          </div>
        </div>
